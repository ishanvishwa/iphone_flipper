...
5.21 V3.0 Phase 4 adds score-based route scheduling for DB-backed worker routes. Route priority is derived from bounded existing signals only: operator `priority`, route due-age/revisit-age, `avg_result_count`, `profitable_hit_rate`, `recent_duplicate_ratio`, and route-health penalties from failure/status state.
5.22 Phase 4 auto-assigns each DB route to `hot`, `warm`, or `sweep`, persists `computed_lane`, `effective_lane`, `priority_score`, and `priority_score_updated_at`, and allows operators to pin `lane_override` through the existing route API/GUI. Effective lane is `lane_override` when present, otherwise `computed_lane`.
5.23 When both `ENABLE_ROUTE_LANES=1` and `ENABLE_PRIORITY_SCHEDULER=1`, the worker chooses the next due DB route by `(effective_lane, priority_score, next_run_at, priority, route_name)` instead of the legacy `(next_run_at, priority, route_name)` order. Env-fallback routes deliberately remain on the legacy path.
5.24 Phase 4 also ranks route queries by lane intent and recent duplicate pressure, acquires a short-lived Redis per-query lock (`query-lock:<sha1>`) before dispatch, skips locked candidates instead of blocking, and records scheduler skips as WAIT outcomes rather than scrape failures.
5.25 Route cadence still honors the existing per-route interval settings as the upper bound. Phase 4 only stretches revisit timing by lane (`hot=1.0`, `warm=1.35`, `sweep=2.0`) and existing status/cooldown multipliers; it does not shorten hot-route cadence below the configured route interval.
...
- `server/services/api/app/main.py` (health, listing snapshot, WebSocket stream, `worker_routes` + `worker_health` endpoints, route lane/score serialization)
