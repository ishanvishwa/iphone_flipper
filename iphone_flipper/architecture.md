...
- Current baseline remains Redis pub/sub + API WebSocket endpoint + GUI polling sync, with V3.0 Phase 1 adding optional Redis Streams dual-write after persistence behind `ENABLE_REDIS_STREAM_EVENTS`; later phases may formalize Redis Streams-first consumers and WebSocket-first desktop sync only after approval.
...
7. API service relays events through Redis/WebSocket-compatible channels; Redis pub/sub remains the live API fanout path in Phase 1, and desktop currently consumes server changes via incremental cursor polling with direct WebSocket apply still pending.
