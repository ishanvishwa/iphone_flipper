...
- The blueprint-alignment pass now binds the warm BrowserContext directly to the claimed `profiles.user_data_dir`, scopes rollout to allowlisted families (default `iphone_broad`), requires validated variants for family execution, and adds a Redis-backed global DOM circuit breaker.
...
3. Each loop iteration selects its next **due** route from persisted scheduler state rather than in-memory round-robin. In the legacy path this state lives on worker-owned `worker_routes`; when `ENABLE_CENTRAL_ROUTE_DISPATCH=1` it lives on centrally managed `central_routes` plus canonical `route_queries`.
...
2.3 If persisted routes exist but are temporarily unavailable (cooldown/query lock/profile cooldown), worker waits in scheduled/cooldown state rather than falling back to `env_default`.
2.4 `env_default` fallback is now used only on the legacy worker-owned path when central dispatch is disabled and no DB routes exist for that worker.
2.5 With central dispatch enabled, each worker first leases one eligible local execution profile from `execution_profiles`, then asks the scheduler for the best due central route, ranks that route's canonical queries, acquires a distributed query lock, and executes only that query on the leased local profile.
...
5.12 V3.0 Phase 0 adds a Redis-backed feature-flag registry (`flipper:flags`) with all upgrade-path flags defaulting to disabled: `ENABLE_REDIS_STREAM_EVENTS`, `ENABLE_NOTIFICATION_CONSUMER`, `ENABLE_GUI_WEBSOCKET_PUSH`, `ENABLE_PRIORITY_SCHEDULER`, `ENABLE_ROUTE_LANES`, and `ENABLE_BACKGROUND_ENRICHMENT`.
...
5.14 V3.0 Phase 1 adds an optional post-persistence Redis Streams dual-write path on `stream:listings` using `XADD ... MAXLEN ~ 10000`. The stream event schema is intentionally flat and compact (`schema_version`, `event_name`, `listing_id`, `worker_name`, `route_name`, `query`, `query_index`, `query_total`, `query_shard_key`, `persisted_at`, `price`, `potential_profit`, `title`, `url`, `thumbnail_url`, `source`) and reuses existing runtime metadata rather than inventing new persisted route/query IDs.
...
5.18 V3.0 Phase 3 upgrades `/ws/listings` from a heartbeat-only stub to a normalized live listing feed behind `ENABLE_GUI_WEBSOCKET_PUSH`. The API still listens to Redis pub/sub as the fanout trigger, but now fetches the canonical Postgres listing row by `listing_id`, normalizes it through a JSON-safe encoder, and broadcasts a `listing_snapshot` payload that mirrors `/listings` item shape and carries `seq_id` as the GUI cursor.
5.19 Phase 3 also adds API-side WebSocket connection management with per-client send serialization, application-level `ping` / `pong` heartbeats, timeout-based dead-client cleanup, and live flag-disable handling that proactively sends `websocket_disabled` and closes connected desktop clients so polling fallback activates immediately.
5.20 Final Phase 3 rollout hardening also covers two acceptance-path issues discovered during production smoke:
  - desktop WebSocket client explicitly replies to websocket control `PING` frames in addition to JSON `ping` heartbeats
  - poll fallback tolerates transient REST/WebSocket errors during API restarts and keeps retrying until backfill and live reconnect succeed
5.21 V3.0 Phase 4 introduced bounded score-based route scheduling from existing signals only: operator `priority`, route due-age/revisit-age, `avg_result_count`, `profitable_hit_rate`, `recent_duplicate_ratio`, and route-health penalties from failure/status state.
5.22 The subsequent Phase 4 alignment addendum makes that scheduler authoritative through centrally managed `central_routes`, canonical `route_queries`, and local `execution_profiles`, gated by `ENABLE_CENTRAL_ROUTE_DISPATCH`. Routes and queries are no longer owned by workers; workers are generic executors that lease only safe local profiles.
5.23 Central routes still carry route-level `hot` / `warm` / `sweep` lanes and persisted `computed_lane`, `effective_lane`, `priority_score`, and `priority_score_updated_at`. Query metadata is now canonical and centralized per route, but route metadata remains the primary scoring/dispatch unit in this phase.
5.24 When `ENABLE_CENTRAL_ROUTE_DISPATCH=1`, each worker leases one eligible local execution profile, then selects the best due central route globally, ranks that route's canonical queries, acquires a short-lived Redis per-query lock (`query-lock:<sha1>`), and executes only the locked query on the leased profile. Existing profile cooldown, manual-login, quiet-hours, and reuse-safety rules remain unchanged.
5.25 Phase 4 alignment also adds explicit anti-starvation: every `CENTRAL_ROUTE_EXPLORATION_EVERY_N` eligible central dispatches, the scheduler prefers due non-hot work when any exists; otherwise it falls back to the normal highest-priority route. Route cadence still honors the configured route interval as the upper bound and only stretches lower-priority revisit timing (`hot=1.0`, `warm=1.35`, `sweep=2.0`) rather than shortening hot cadence below baseline.
5.25.1 Legacy `worker_routes` endpoints and tables remain available only as migration/audit helpers during rollout. The authoritative write path for scheduler state is now the central route API (`/routes`, `/routes/{route_name}/queries`) plus execution-profile visibility (`/execution-profiles`).
5.26 V3.0 Phase 5 adds optional background enrichment behind `ENABLE_BACKGROUND_ENRICHMENT`. The worker now splits listing persistence into a decision-ready hot path (`id`, `title`, `price`, `url`, `location`, `thumbnail_url` when cheaply available, `model`, `condition`, `max_buy_price`, `potential_profit`, `status`, timestamps) and a deferred cold path (`description`, `seller_name`, thumbnail backfill).
5.27 Phase 5 keeps Postgres-first persistence and existing event fanout semantics intact: the hot row is persisted first, Redis pub/sub and existing stream fanout continue immediately after persistence, and cold enrichment work is queued separately onto `stream:listing_enrichment` only when the listing is new, its cold-field fingerprint changed, or required enrichable fields remain missing after a failed/unknown prior enrichment state.
5.28 Deferred enrichment uses a dedicated compose-wired `enrichment_worker.py` consumer-group service over `stream:listing_enrichment` with group `listing_enrichment`, `XREADGROUP`, `XAUTOCLAIM`, bounded retry/backoff, PostgreSQL ledger `listing_enrichment_ledger`, and terminal listing bookkeeping fields (`enrichment_status`, `enrichment_source_hash`, `enriched_at`, `enrichment_last_error`). Successful enrichment republishes a lightweight pub/sub `listing_updated` trigger so API/WebSocket clients can merge late cold-field updates onto the existing listing row without inventing a second public payload contract.
...
5.33 V4 query-family rows represent future schedulable search intent independent of worker ownership. Variants are seeded from existing `route_queries.query_text`, but runtime execution now only uses enabled variants whose `validation_state='validated'`; when present, `url_template` is used as the navigation target for that validated variant.
...
7.3 V3.0 Phase 4 extends worker telemetry with scheduler-specific correlation data (`lane_override`, `computed_lane`, `effective_lane`, `priority_score`, compact score components, selected query, query lock key/status, route due-age, route revisit-age, and lane interval multiplier) so rollout can be measured without adding a second telemetry system.
7.4 V3.0 Phase 5 keeps `/listings` and WebSocket `listing_snapshot.item` backward compatible and adds only nullable `thumbnail_url` to the public listing shape. Desktop sync now accepts same-cursor websocket updates when `updated_at` is newer or cold fields were previously missing, so later enrichment merges into the same local SQLite row instead of being dropped just because `seq_id` did not advance.
7.5 V3.0 Phase 6 adds authenticated operational inspection endpoints on the API without changing listing payloads: `/ops/realtime-health`, `/ops/stream-backlog`, `/ops/runtime-config`, `/ops/notification-dead-letter`, and `/ops/replay/notifications`. The API now reports websocket connection count, worker publish-health state from `worker_heartbeats`, per-stream backlog and group lag (`stream:listings`, `stream:listing_enrichment`, `stream:notification_dead_letter`), and the current Redis-backed runtime controls.
8. Worker runtime can still send immediate Telegram listing cards on `listing_created` when `potential_profit >= TELEGRAM_NOTIFY_MIN_PROFIT` (default `0`) and model is not blank/`Unknown`, including link + price/profit/description summary, but this is now a fallback path used only when the dedicated notification consumer is disabled or stream delegation is unavailable. Telegram delivery remains text-only; thumbnails are stored/synced for GUI use but are not included in notification formatting or media-send behavior.
...
9.2 Phase 6 adds operational controls to that notification service rather than replacing it: `NOTIFICATION_CONSUMER_DRAIN` lets operators finish claimed work while pausing new reads, dead-letter writes protect the consumer group from poison-message backlog, and replay is intentionally limited to dead-letter entries so operators can recover failed alerts without introducing a general stream rewind path.
...
8.10 The blueprint alignment pass closes the remaining production-safety gaps for V4 rollout: the warm session now launches from the claimed profile directory rather than a second Dolphin-selected identity; V4 family claims are rollout-scoped through `V4_ROLLOUT_FAMILY_ALLOWLIST` (default `iphone_broad`); the broad family is held to `IPHONE_BROAD_MIN_GAP_SECONDS=5` with `IPHONE_BROAD_INITIAL_VARIANTS=1`; notification delivery accepts `listing_updated` only for `dedupe_kind='price_change'` using a distinct delivery-ledger key; and repeated `DOM_CHANGED` signals now open a Redis-backed global pause window (`V4_DOM_CHANGED_THRESHOLD`, `V4_DOM_CHANGED_WINDOW_SECONDS`, `V4_DOM_GLOBAL_PAUSE_SECONDS`) instead of letting workers continue to burn healthy profiles against broken selectors.
...
- `server/services/worker/lease_manager.py`, `server/services/worker/route_transitions.py`, `server/services/worker/scheduler.py`, `server/services/worker/persona.py` (worker modularization, Phase 4 route scoring, lane scheduling, and Redis query locks)
- `server/services/common/route_lanes.py` (shared route-lane validation/normalization for worker + API)
...
Legacy remote VPS scraper route definitions, retained for migration/audit helpers:
...
#### `central_routes` (Postgres)

Authoritative centrally managed scheduler routes for aligned Phase 4 dispatch:

- global `route_name` primary key
- legacy origin metadata: `legacy_worker_name`, `legacy_route_name`
- route controls: `is_enabled`, `priority`, `route_interval_seconds`
- proxy preferences: `proxy_server`, `proxy_username`, `proxy_password`, `proxy_mode`, `proxy_pool`
- route health/state: `status`, `status_reason`, `status_since`, `consecutive_failures`, `cooldown_until`, `last_error`
- scheduler state: `next_run_at`, `lane_override`, `computed_lane`, `effective_lane`, `priority_score`, `priority_score_updated_at`
- derived route metrics: `avg_result_count`, `profitable_hit_rate`, `recent_duplicate_ratio`, `avg_page_load_ms`, `successful_cycles`, `last_selected_at`, `last_success_at`

#### `route_queries` (Postgres)

Canonical query set attached to each central route:

- `route_name`, `query_text`, `query_order`, `is_enabled`
- per-query scheduler signals: `last_selected_at`, `last_success_at`, `avg_result_count`, `profitable_hit_rate`, `recent_duplicate_ratio`, `selection_count`, `last_error`

#### `execution_profiles` (Postgres)

Worker-local execution/safety inventory used by central dispatch:

- identity: `user_data_dir`, `worker_name`
- availability: `is_enabled`, `status`, `status_reason`, `status_since`, `cooldown_until`
- manual-login/quarantine state: `manual_login_required`, `manual_login_reason`, `manual_login_required_at`, `quarantined_at`, `quarantine_reason`, `quarantine_evidence`
- runtime history: `last_selected_at`, `last_success_at`, `consecutive_failures`, `last_error`
...
- effective route snapshot fields for env/legacy/central visibility: `route_source`, `route_user_data_dir`, `route_search_queries`, proxy snapshot fields
- event publish health fields: `last_event_publish_at`, `last_event_publish_status`, `last_event_publish_error`, `last_stream_event_id`
...
8. **Desktop sync mode**: GUI now supports WebSocket-first server sync with poll fallback, reconnect replay from the persisted `server_sync_since_id` watermark, and selection/focus preservation during sync-driven table refreshes.
...
11. Keep your desktop app machine always able to reach the VPS API endpoint (HTTPS on `443`) and the live WebSocket endpoint (`wss://.../ws/listings`) now that Phase 3 desktop sync is enabled.
...
5. Wire desktop GUI to consume server updates without run-end refresh dependency (implemented baseline: WebSocket-first with reconnect replay and poll fallback).
...
1. This version reflects codebase state audited on **2026-03-10** and includes all prior GUI/process enhancements (price-sheet editor, row flags, scraper progress/cancel, hybrid GraphQL+DOM fetch, accessory suppression, proxy ingestion/ban controls, VPS route rendering), the implemented 24x7 server baseline (due-time scheduling, WAIT outcomes, profile/query-shard locks, proxy health scoring, route state machine, signal detection, quarantine operations, proxy-binding verification, persona variation, worker modularization, shared schema ensure, minimum-route resilience, structured telemetry), **plus** the following recent additions: scraper module refactoring from monolithic `scraper.py` to structured package (`scraper/`), standalone event-driven notification worker with FCM push and priority-tier dispatch (`notification_worker.py`), proxy provider real-time health monitoring (`proxy_monitor.py`) now actively wired into worker loop, Dolphin Anty profile management GUI tab with API key integration and Bearer-token authentication, manual standalone patching of `scraper/core.py` utilizing `patch_core.py` and `scraper/legacy_utils.py` (814-line V1 utility preservation), **V2.2 milestone**: `core/scraper/` package rewrite with Dolphin Anty CDP-based browser launch (`connect_over_cdp`) with `DOLPHIN_WS_HOST` Docker→host connectivity and duplicate-running profile reuse (`E_BROWSER_RUN_DUPLICATE`), weighted bucket query diversification, quiet hours scheduling, session duration caps, dynamic Dolphin profile cloud-first allocation via Postgres locks with three-tier API fallback, VPS infrastructure migration to `ubuntu@15.235.185.32`, Caddy TLS reverse proxy in Docker Compose, `DOLPHIN_ANTY_TOKEN` compose environment for cloud API authentication, **V2.3**: ~~5-slot concurrent worker architecture~~ removed — reverted to single-loop-per-container (dev-log §34); parallelism via Docker worker containers, **V3.0 Phase 0**: Redis-backed upgrade-path feature flags in `flipper:flags`, JSON latency/correlation logging across worker/API/GUI poll-sync flows, extended cycle telemetry aggregates, **V3.0 Phase 1**: post-persistence Redis Streams dual-write on `stream:listings` with capped retention (`MAXLEN ~ 10000`), compact flat event schema, meaningful-change suppression for unchanged updates, and non-fatal stream publish telemetry while Redis pub/sub remains the active API fanout path, **V3.0 Phase 2**: worker-side exact-once hardening for new-listing stream publication plus a compose-wired notification consumer using Redis Streams consumer groups, PostgreSQL delivery ledger dedupe, restart-safe pending recovery, and delegated Telegram delivery outside the worker hot path, **V3.0 Phase 3**: normalized API WebSocket listing snapshots, JSON-safe websocket payload encoding, connection-managed `ping`/`pong` heartbeats, desktop `aiohttp` live-sync coordinator with replay-window catch-up, poll fallback, transient-error retry hardening, reconnect backoff, and GUI selection preservation during sync-driven refresh, **V3.0 Phase 4**: central route ownership with canonical route queries, execution-profile-gated workers, exploration-reserve dispatch, shared lane validation, Redis per-query locks, central route/execution-profile API surfaces, and production-safe text-only notifications, **V3.0 Phase 5**: fast-path listing persistence with deferred cold-field enrichment, nullable `thumbnail_url` carried through API/WebSocket/desktop sync, durable enrichment queue/ledger infrastructure, compose-wired `enrichment_worker`, and later same-cursor GUI merges for post-alert description/seller/thumbnail updates, **and V3.0 Phase 6**: Redis-backed runtime operator config (`flipper:runtime_config`), authenticated API ops endpoints for flag/runtime updates and backlog inspection, worker publish-health persistence, notification dead-letter handling on `stream:notification_dead_letter`, bounded dead-letter replay back onto `stream:listings`, and deterministic drain-mode controls for the notification consumer.
2. V3.0 Phase 0 intentionally did **not** change the current runtime contracts at the time it landed: worker pub/sub stayed in place, inline worker Telegram notifications stayed active, the API WebSocket endpoint remained available, the GUI remained polling-first, and the notification-worker payload-shape mismatch was documented but deferred. Those deferred desktop-sync changes are now implemented in Phase 3, while the explicit non-goals around scraper transport, token replay, escalation heuristics, and pre-persistence notification ordering remain unchanged.
